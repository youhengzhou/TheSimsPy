...
class WaterDrinker(Char):
    def createEvents(self, simTime, futureEventList, chars):
        if not any(self in evt.chars for evt in futureEventList):
            if self.state == "idle":
                heapq.heappush(
                    futureEventList, DrinkWater0(simTime + random.randint(1, 2), [self])
                )

            if self.state == "need to piss":
                heapq.heappush(
                    futureEventList, PissWater0(simTime + random.randint(1, 2), [self])
                )
                self.pissCounter += 1
...
class DrinkWater0(Event):
    ...
    def handleEvent(self, futureEventList):
        ...
        heapq.heappush(
            futureEventList, DrinkWater1(self.time + random.randint(1, 50), self.chars)
        )
...
class PissWater0(Event):
    ...
    def handleEvent(self, futureEventList):
        ...
        heapq.heappush(
            futureEventList, PissWater1(self.time + random.randint(1, 50), self.chars)
        )
